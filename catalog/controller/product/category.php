...
class ControllerProductCategory extends Controller
{
    public function index()
    {
        ...
        if (isset($this->request->get['d'])) {
            $filter_product['d'] = explode(';', $this->request->get['d']);
        }

        if (isset($this->request->get['D1'])) {
            $filter_product['D1'] = explode(';', $this->request->get['D1']);
        }

        if (isset($this->request->get['L'])) {
            $filter_product['L'] = explode(';', $this->request->get['L']);
        }
        ...
        if ($category_info) {
            ...
            foreach ($results as $result) {
                ...
                foreach ($trElements as $node) {
                    ...
                    $node_doc->loadHTML('<?xml encoding="utf-8" ?>' . $nodeHTML);
                    $nodeList = $node_doc->getElementsByTagName('td');

                    $attributeValue =
                        $nodeList->item(0)->nodeValue . ' - ' .
                        $nodeList->item(1)->nodeValue . '<br>';

                    if (strpos($attributeValue, 'Диаметр проволоки') !== false) {
                        $description .= preg_replace('/\(.+\)/', '', $attributeValue);;
                    }
                    if (strpos($attributeValue, 'Наружный диаметр пружины') !== false) {
                        $description .= preg_replace('/\(.+\)/', '', $attributeValue);;
                    }
                    if (strpos($attributeValue, 'Длина пружины') !== false) {
                        $description .= preg_replace('/\(.+\)/', '', $attributeValue);;
                    }
                    ...
                    if (strpos($attributeValue, 'Длина навитой части пружины') !== false) {
                        $description .= preg_replace('/\(.+\)/', '', $attributeValue);;
                    }
                    if (strpos($attributeValue, 'Зазор') !== false) {
                        $description .= preg_replace('/\(.+\)/', '', $attributeValue);
                    }
                }
